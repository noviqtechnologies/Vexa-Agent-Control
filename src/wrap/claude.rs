...
use super::{backup, config_path, transformer, WrapError};
...
/// Wrap Claude Desktop MCP servers with AgentControl (FR-304).
...
pub fn wrap_claude(dry_run: bool, scan_responses: bool) -> Result<WrapResult, WrapError> {
    ...
    // 11. Bootstrap ~/.agentcontrol/config.yaml
...
/// Create ~/.agentcontrol/config.yaml on first wrap if it doesn't exist.
    ...
    if !config_file.exists() {
        ...
        let content = format!(
            "# AgentControl configuration\n\
             ...
             log_path: ~/.agentcontrol/audit.log\n",
            scan
        );
...
fn print_dry_run_diff(
    ...
    println!(
        "{} {}",
        "🔍".blue(),
        "DRY-RUN MODE — no changes will be written".yellow().bold()
    );
    println!(
        "{} Config: {}",
        "  →".dimmed(),
    ...
    if let (Some(before_servers), Some(after_servers)) = (
        ...
        for (name, after) in after_servers {
            ...
            println!(
                "    {} command: {} → {}",
                "-".red(),
                before["command"].to_string().red(),
                after["command"].to_string().green()
            );
            println!(
                "    {} args:    {} → {}",
                "~".yellow(),
                serde_json::to_string(&before["args"])
                    .unwrap_or_default()
                    .red(),
                serde_json::to_string(&after["args"])
                    .unwrap_or_default()
                    .green(),
            );
        }
    ...
    println!(
        ...
        if scan_responses {
            ...
            "DISABLED (use --scan-responses to enable)"
                .yellow()
                .to_string()
        }
    );
...
fn print_force_unwrap_instructions(config_path: &Path) {
    println!(
        "{} No backup found. Manual cleanup instructions:",
        "⚠".yellow().bold()
    );
    println!(
...
pub fn print_wrap_summary(result: &WrapResult) {
    println!(
        "{} {}",
        "✔".green().bold(),
        "AgentControl wrapped Claude Desktop.".green().bold()
    );
    println!(
        "  {} Config:            {}",
        "→".dimmed(),
...
pub fn print_unwrap_summary(result: &UnwrapResult) {
    println!(
        "{} {}",
        "✔".green().bold(),
        "AgentControl removed from Claude Desktop.".green().bold()
    );
    println!(
        "  {} Config restored:   {}",
        "→".dimmed(),
