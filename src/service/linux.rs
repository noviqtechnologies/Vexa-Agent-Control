...
"#,
    ...
    fs::write(unit_path, service_content)
        .map_err(|e| format!("failed to write systemd unit file (root permissions required?): {}", e))?;
    ...
    let enable = Command::new("systemctl")
        ...
        .output();
    if let Err(e) = enable {
        return Err(format!("failed to execute systemctl enable --now agentwall: {}", e));
    }

    println!("{} AgentWall Linux systemd service installed and started!", "✔".green().bold());
    Ok(())
}

pub fn uninstall_linux_service() -> Result<(), String> {
    let _ = Command::new("systemctl").args(["stop", "agentwall"]).output();
    let _ = Command::new("systemctl").args(["disable", "agentwall"]).output();
    ...
    println!("{} AgentWall Linux systemd service uninstalled.", "✔".green().bold());
    Ok(())
}
