...
.DESCRIPTION
    ...
    and applies Terraform infrastructure with optimized parallelism (-parallelism=20).
.PARAMETER SkipBuild
...
function Test-ImageExists {
    ...
    $check = & gcloud artifacts docker images describe $ImageTag --project=$ProjectId 2>&1
...
$GwHash  = Get-DirSourceHash -Paths @("$RepoRoot\src", "$RepoRoot\benches", "$RepoRoot\control-plane\proto") -ExtraFiles @("$RepoRoot\Cargo.toml", "$RepoRoot\Cargo.lock", "$RepoRoot\Dockerfile")
...
Write-Host "`n[3/4] Applying Terraform with High Parallelism (-parallelism=20)..." -ForegroundColor Yellow
